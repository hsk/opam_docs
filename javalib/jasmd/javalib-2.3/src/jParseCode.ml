...
let other count ch length instr =
  match instr with
    ...
    | OpTableSwitch (def, low, high, tbl) ->
        ...
        (*Format.printf "; table switch length %d calclen %d array length %d padding_size %d c %d\n"
          	length
          	(13 + padding_size + 4 * (Array.length tbl))
          	(Array.length tbl)
          	padding_size
          c;*)
    ...
    | OpLookupSwitch (def, tbl) ->
        ...
        (*Format.printf "; lookup length %d  calclen %d array length %d\n"
          length 
          	(9 + padding_size + 8 * (List.length tbl))
          	(List.length tbl)
          	;*)
